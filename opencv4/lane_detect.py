...
def get_instruction(angle):
    instruction = ""
    if angle < 10:
    	instruction = "go"
    if angle >10 and middle_line[0] > 320:
    	instruction = "turnleft"
    if angle >30 and middle_line[0] > 320:
    	instruction = "rotleft"
    if angle > 10 and middle_line[0] < 320:
...
while True:
    ...
    if lines is not None:
        # find fine lines
        lines_all = lines_fine(cap, lines)
        angle = get_angle()
        print(angle)
        instruction = get_instruction(angle)
        print(instruction)
